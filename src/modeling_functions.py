...
def create_choropleth_map_pred(df1, df2, df3, col1, col2, col3, cmap, title, ptcolor):
    ...
    Select df2 and col2 for positive class
...
# function that visualizes predictions and overlays FRESH geometry
def create_choropleth_map_pred_fresh(df1, df2, df3, df4, col1, col2, col3, col4, cmap, title, ptcolor):
    '''Creates a choropleth map with four dataframes for plotting predictions
    Select df1 and col1 for negative class
    Select df2 and col2 for positive class
    Select df3 and col3 for predicted positive class
    Select df4 and col4 for FRESH geometry to overlay
    Enter cmap for desired color palette
    Enter title for map title and image saved name'''
    ...
    ax.annotate("•", xy=(0.78, .855), size=90, xycoords='figure fraction', color='green', alpha=0.3)
    ax.annotate("FRESH", xy=(0.80, .86), size=70, xycoords='figure fraction')
